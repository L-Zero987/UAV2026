 ...
 */
pid_alg_c::pid_alg_c(PID_Init_Config_t pid_config) : d_filter(pid_config.d_filter_num),
                                                     out_filter(pid_config.out_filter_num)
{
    // memset(&this->pid_measure_, 0, sizeof(pid_alg_n::pid_parameter_t));
    this->pid_measure_ = {};
    ...
    this->pid_measure_.threshold_min = pid_config.threshold_min;
...
// 无参构造函数
pid_alg_c::pid_alg_c() : d_filter(0), out_filter(0)
{
}
...
float pid_alg_c::ECF_PID_Calculate(float SetValuew)
{
    ...
    if (this->ActualValueSource_ == NULL)
        this->pid_measure_.ActualValue = 0;
    else
    ...
    if (abs(this->pid_measure_.error) >= this->pid_measure_.deadband) // 死区
    {
    ...
    this->pid_output = this->pid_measure_.out;
 ...
 * @retval         none
 */
float pid_alg_c::ECF_PID_Calculate(float SetValuew, float ActualValue)
{
    float thiserr = 0; // 积分增值
    ...
    this->pid_measure_.ActualValue = ActualValue;
...
{
    if (std::abs(this->pid_measure_.error) <= this->pid_measure_.errorabsmin) // 最小值
    {
        return 1.0f;
    }
    else if (std::abs(this->pid_measure_.error) > this->pid_measure_.errorabsmax) // 最大值
    {
        return 0.0f;
    }
    else
    {
        return (this->pid_measure_.errorabsmax - abs(this->pid_measure_.error)) / (this->pid_measure_.errorabsmax - this->pid_measure_.errorabsmin);
    }
}
...
{
    float kFactor = 0.0f;
    if (abs(this->pid_measure_.LastSetValue - this->pid_measure_.SetValue) <= this->pid_measure_.stepIn)
    {
        return;
    }
    else
...
// 修改步长
void pid_alg_c::ECF_PID_Reset_StepIn(float StepIn)
{
    this->pid_measure_.stepIn = StepIn;
}
...
float *pid_alg_c::ECF_PID_GetActValSource(void)
{
    if (this->ActualValueSource_ != NULL)
...
// 串级PID构造函数
cascade_pid_c::cascade_pid_c(PID_Init_Config_t outer_pid_config, PID_Init_Config_t inter_pid_config, float output_limit)
    : Output_Limit(output_limit), outer_pid(outer_pid_config), inter_pid(inter_pid_config)
{
}

// 清除PID
void cascade_pid_c::Cascade_pid_Clear()
{
    inter_pid.ECF_PID_CLEAR();
    outer_pid.ECF_PID_CLEAR();
}

// 串级pid计算函数（已传入反馈指针版
float cascade_pid_c::Cascade_pid_Calculate(const float *SetPosVal)
{
    Target = SetPosVal;
    // 计算外环
    outer_pid_output = (Outer_ActualVal == nullptr) ? outer_pid.ECF_PID_Calculate(*SetPosVal, 0) : outer_pid.ECF_PID_Calculate(*SetPosVal, *Outer_ActualVal);
    // 加入速度前馈
    if (SpeedFeedWard != nullptr)
        outer_pid_output += *SpeedFeedWard;
    // 计算内环
    inter_pid_output = (Inter_ActualVal == nullptr) ? inter_pid.ECF_PID_Calculate(outer_pid_output, 0) : inter_pid.ECF_PID_Calculate(outer_pid_output, *Inter_ActualVal);
    user_value_limit(inter_pid_output, -Output_Limit, Output_Limit);
    return inter_pid_output;
}

// 串级pid计算函数 （手动传值版
float cascade_pid_c::Cascade_pid_Calculate(const float *SetPosVal, const float *outer_ActValSource, const float *inter_ActValSource)
{
    Target = SetPosVal;
    // 计算外环
    outer_pid_output = (outer_ActValSource == nullptr) ? outer_pid.ECF_PID_Calculate(*SetPosVal, 0) : outer_pid.ECF_PID_Calculate(*SetPosVal, *outer_ActValSource);
    // 加入速度前馈
    if (SpeedFeedWard != nullptr)
        outer_pid_output += *SpeedFeedWard;
    // 计算内环
    inter_pid_output = (inter_ActValSource == nullptr) ? inter_pid.ECF_PID_Calculate(outer_pid_output, 0) : inter_pid.ECF_PID_Calculate(outer_pid_output, *inter_ActValSource);
    user_value_limit(inter_pid_output, -Output_Limit, Output_Limit);
    return inter_pid_output;
}

// 传入实际值指针
void cascade_pid_c::Cascade_SetValPoint(const float *outer_ActValSource, const float *inter_ActValSource)
{
...
// 传入速度前馈指针
void cascade_pid_c::Cascade_SetSpeedfeedward(const float *speed)
{
    SpeedFeedWard = speed;
}
